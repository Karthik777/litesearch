...
# %% auto #0
__all__ = ['BUSY_TIMEOUT_MS', 'SCHEMA_RETRIES', 'RERANK_FANOUT', 'embed_chunk', 'busy_window', 'db_lock', 'ConnectionBusy',
           'write_txn', 'process_content', 'retry_schema', 'upsert_all', 'rrf_merge', 'database', 'rerank_hits']
...
import apsw, apsw.bestpractice, threading, weakref, time
...
def retry_schema(f):
    'Retry a read whose statements another connection invalidated mid-flight.'
    @wraps(f)
    def _f(*a, **kw):
        for i in range(SCHEMA_RETRIES):
            try: return f(*a, **kw)
            except apsw.SchemaChangeError:
                if i == SCHEMA_RETRIES - 1: raise
                time.sleep(0.02 * (i + 1))
    return _f
