...
NP_DTYPE = {'i8':np.int8, 'f16':np.float16, 'f32':np.float32, 'f64':np.float64}

def sql_in(col, vals): return f'{col} in ({",".join(map(repr, vals))})'  # SQL IN clause; callers guard against empty vals

_FTS_TOKENIZE = 'porter simplify casefold 1 sanskrit unicodewords'

class StoreMismatch(ValueError):
    'A store reopened with a different encoder or tokenizer than the one that wrote it.'
...
def content_id(content): return hash_record({'content': content}) if content else None
...
def get_store(self:Database,        # database connection
    ...
    self._stamp_store(name, encoder=encoder, tokenize=tok)
...
# %% ../nbs/01_core.ipynb #6e5d57548ddf440c
STORE_STAMP = ('encoder', 'tokenize')

@patch
def _store_meta(self:Database, name:str):
    'What wrote this store, or None when the store predates the stamp.'
    return first(self.t.store_meta(where=f'name={name!r}')) if 'store_meta' in self.t else None

@patch
def _stamp_store(self:Database, name:str, **kw):
    'Record the encoder and tokenizer a store was created with; raise when a reopen disagrees.'
    self.t.store_meta.create(name=str, encoder=str, tokenize=str, created_at=float, pk='name',
                             if_not_exists=True, defaults=dict(created_at='CURRENT_TIMESTAMP'))
    now, was = {k: str(kw.get(k) or '') for k in STORE_STAMP}, self._store_meta(name)
    if was is None: return self.t.store_meta.insert(dict(name=name, **now), ignore=True)
    # an unnamed side never conflicts: a custom encoder is unknowable, not wrong
    bad = {k: (was[k], now[k]) for k in STORE_STAMP if was[k] and now[k] and was[k] != now[k]}
    if bad: raise StoreMismatch(
        f'store {name!r} holds ' + ', '.join(f'{k}={o!r}, not {n!r}' for k, (o, n) in bad.items())
        + '. Two encoders in one index do not compare, and a query stems to what the index does not'
          ' hold: use a new store name, or rebuild this one.')
...
def rowid_sel(): return 'rowid as rowid'
@patch
def ann_search(self:Table,          # store table (must be ANN-registered)
    ...
    sel = ','.join(cols + [rowid_sel()]) if cols else '*, %s'%rowid_sel()
    wh = f'where {sql_in("rowid", keys)}' + (f' AND {where}' if where else '')
...
def _sync_index(self:Table, add_ids, rm_rowids, dtype=np.float16, save=True):
    ...
    with db_lock(self.db):
        ...
        kw = dict(select=f'{rowid_sel()}, embedding', where=sql_in('id', add_ids))
...
def sync(self:Table,            # store table (hash-id; ANN mirrored if registered)
    ...
    with db_lock(self.db):
        ...
        cont_hash = {content_id(d['content']): d for d in content}
        if force: ex = set()
        elif key_col:
            vals = {v for d in content if (v:=d.get(key_col)) is not None}
            kw = dict(select='id', where=sql_in(key_col, vals))
        ...
        m  = self.db._ann_meta(self.name)
        dt = NP_DTYPE.get(m['dtype'], np.float16) if m else np.float16
        rm_rowids = L(self(select=rowid_sel(), where=sql_in('id', stale))).itemgot('rowid') if (m and stale) else L()
        if stale: self.delete_where(where=sql_in('id', stale))
...
def rebuild_index(self:Table, dtype=None):
    ...
    with db_lock(self.db):
        ...
        dt = dtype or NP_DTYPE.get(m['dtype'], np.float16)
        rows = L(self(select=f'{rowid_sel()}, embedding')).filter(lambda r: r['embedding'])
...
def ann_vec(self:Table,          # ANN-registered store
    ...
    return np.asarray(idx[key], dtype=NP_DTYPE.get(m['dtype'], dtype)).reshape(-1)
