...
# %% auto #0
__all__ = ['BUSY_TIMEOUT_MS', 'RERANK_FANOUT', 'embed_chunk', 'busy_window', 'db_lock', 'ConnectionBusy', 'write_txn',
           'process_content', 'upsert_all', 'rrf_merge', 'database', 'rerank_hits']
...
def _slug(content): return hash_record({'content': content}) if content else None
...
def db_lock(db):
    ...
    It cannot help a thread that runs SQL on the same connection outside those paths, and a caller
    that composes several litesearch calls into one logical read is such a thread. Use
    `Database.clone()` to give each thread its own connection instead.'''
...
class ConnectionBusy(RuntimeError):
    'Another thread is running SQL on this connection outside the paths `db_lock` serialises.'

def _busy_elsewhere(db):
    live = [t.name for t in threading.enumerate() if t is not threading.current_thread()]
    return ConnectionBusy(
        'this connection is busy in another thread, so the writer here is the victim and not the cause. '
        '`db_lock` serialises litesearch\'s own read-modify-write paths only: a caller that runs SQL '
        'around them, or composes several litesearch calls into one logical read, is not serialised. '
        f'Give the other thread its own connection with `Database.clone()`. Other live threads: {live}.')
...
def write_txn(db):
    ...
    c = db.conn
    with db_lock(db):
        # this check is the first touch of the connection, so it is where an unserialised
        # reader on another thread surfaces. apsw blames the writer; say who is actually at fault.
        try: open_txn = c.in_transaction
        except apsw.ThreadingViolationError as e: raise _busy_elsewhere(db) from e
        if open_txn: yield; return
...
# %% ../nbs/01_core.ipynb #68879629
@patch(as_prop=True)
def ensured(self:Database):
    '''Schema this connection has already created, so idempotent DDL runs once instead of per call.
    Clear it if you drop litesearch tables behind its back on the same connection.'''
    if not hasattr(self, '_ensured'): self._ensured = set()
...
def get_store(self:Database,        # database connection
    ...
    if key in self.ensured: return self.t[name]
...
def clone(self:Database, busy_timeout:int=BUSY_TIMEOUT_MS, **kw) -> Database:
    ...
    re-registered, and the busy timeout is preset rather than left at apsw's 100ms.'''
    f = self.conn.filename
    if not f: raise ValueError('an in-memory database has nothing to reopen: share the one connection, '
                               'and keep every thread inside litesearch\'s own paths so `db_lock` applies')
    return database(f, busy_timeout=busy_timeout, **kw)
