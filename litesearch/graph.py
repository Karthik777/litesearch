...
def _prose_job(txt):
    ...
    Only text crosses the boundary and only tuples come back — no database handle, no spaCy
    pipeline, nothing that a fork would have to keep consistent.'''
    return [[(s, k) for s, k in w] for w in prose_windows(txt, None)]
...
def build_graph(db,                  # Database with a chunk store
    ...
    def prose_wins():
        ...
        Extraction is 88% of a build and is a pure function of the chunk text, so this is a map with
        a cheap serial reduce after it. Processes, not threads: yake and spaCy are both python and
        both hold the GIL. spaCy has its own `n_process`, which is used rather than reinvented —
        it forks around the pipeline instead of shipping one per task.
        ...
        nw = _n_workers(len(prose_q), n_workers)
        if nlp is None:
            if nw and nw > 1:
