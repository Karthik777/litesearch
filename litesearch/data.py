...
# %% auto #0
__all__ = ['skip_folder_re', 'skip_file_re', 'code_exts', 'file_exts', 'PROFILES', 'MIN_PARALLEL_FILES', 'needs_ocr',
           'orphan_vals', 'scrambled_layout', 'clean_md', 'fix_layout', 'ocr_parse', 'oxide_parse', 'pdf_parse',
           'SafeFastChunker', 'chunk_markdown', 'chunk_spans', 'repo_root', 'spec', 'pyparse', 'ipynb_parse',
           'non_py_sigs', 'chunk_texts', 'Profile', 'register_profile', 'profile_for', 'file_parse', 'pkg2files',
           'dir2files', 'pkg2chunks', 'dir2chunks', 'installed_packages', 'clean', 'add_wc', 'mk_wider', 'kw', 'pre',
           'img2png', 'png_det', 'images_to_pdf', 'mv_skill_md']
...
def fix_layout(doc:PdfDocument, page:int, md:str, preserve_layout=False) -> str:
	...
	txt = clean_md(doc.to_plain_text(page, preserve_layout), preserve_layout)
...
def oxide_parse(pdf: str|PdfDocument|bytes|Path,
	...
	def _page(i):
		md = clean_md(pdf.to_markdown(i, preserve_layout, **kw), preserve_layout)
		return fix_layout(pdf, i, md, preserve_layout).replace(f'{imdir}/', f'{image_dir}/')
	pages = [_page(i) for i in range(int(pdf.page_count))]
...
def pdf_parse(pdf: PdfDocument | str | Path | bytes,  # PdfDocument object
	...
	sel = ocr_selection.lower()
	md, ocr_chk = oxide_parse(pdf, out_path, preserve_layout, image_dir)
	if sel == 'off': return md
	if sel == 'on' or ocr_chk:
		ocr = [clean_md(p.text, preserve_layout) for p in ocr_parse(pdf).pages]
		if ''.join(ocr).strip(): return ocr       # an empty OCR pass is a failure, not a document
	return md
