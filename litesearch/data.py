...
# %% auto #0
__all__ = ['py_dir_skip_re', 'py_file_skip_re', 'py_glob', 'skip_py_glob', 'chunk_markdown', 'repo_root', 'spec', 'pyparse',
           'non_py_sigs', 'file_parse', 'pkg2files', 'pkg2chunks', 'installed_packages', 'clean', 'add_wc', 'mk_wider',
           'kw', 'pre', 'img2png', 'png_det', 'images_to_pdf']
...
def non_py_sigs(p):
	def meta_(): return dict(path=p, uploaded_at=Path(p).stat().st_mtime if p else None, lang=p.suffix)
...
def file_parse(p:Path=None,  # path to a code file
    ...
    return pyparse(p, **kwargs) if p.suffix == '.py' else non_py_sigs(p)
...
def pkg2files(pkg:str,								# package name
              file_glob:str=None,				    # file glob to match
	...
	return globtastic(Path(nm.origin).parent,file_glob=file_glob,skip_file_glob=skip_file_glob,
		folder_re=nm.name, skip_folder_re=skip_folder_re, skip_file_re=skip_file_re, func=func, **kwargs)
