...
def add_wc(q:str  # query to be passed for fts search
    ...
    the apostrophe inside `pathfinder's`, and `pathfinder's*` is an FTS5 *syntax error* — so without
    this any query containing a possessive or a contraction made `search` raise instead of search.'''
