...
# %% auto #0
__all__ = ['skip_folder_re', 'skip_file_re', 'code_exts', 'file_exts', 'needs_ocr', 'chunk_markdown', 'repo_root', 'spec',
...
# %% ../nbs/02_data.ipynb #ocr_pdf_cell
def needs_ocr(md:str) -> bool:
	'True when pdf_oxide flagged the page as scanned (no extractable text layer).'
	return '[OCR REQUIRED' in md

@patch
def pdf_ocr_texts(doc:PdfDocument, engine=None, st=0, end=None) -> L:
	'OCR text from each page via pdf_oxide + PaddleOCR (engine auto-set-up with `get_ocr_engine`).'
	if engine is None:
		from litesearch.utils import get_ocr_engine
		engine = get_ocr_engine()
	return L(range(st, ifnone(end, doc.page_count()))).map(lambda i: doc.extract_text_ocr(page=i, engine=engine))

@patch
def pdf2md(doc:PdfDocument,
           st=0,            # start page
           end=None,        # end page (exclusive)
           ocr:str='auto',  # 'auto': OCR only flagged pages; 'on': force OCR; 'off': never OCR
           engine=None      # pdf_oxide OcrEngine; auto-created when needed
) -> L:
	"Markdown per page (fossick-style `pdf2md`): scanned pages fall back to PaddleOCR."
	if ocr == 'on': return doc.pdf_ocr_texts(engine, st, end)
	mds = doc.pdf_markdown(st, end)
	if ocr == 'off' or not any(mds.map(needs_ocr)): return mds
	if engine is None:
		try:
			from litesearch.utils import get_ocr_engine
			engine = get_ocr_engine()
		except Exception as e:
			print(f'OCR required but engine setup failed ({e}); returning raw markdown')
			return mds
	return L(doc.extract_text_ocr(page=st+i, engine=engine) if needs_ocr(md) else md for i,md in enumerate(mds))
...
def pdf_chunks(self:PdfDocument, # PDF document
               ...
               ocr:str='auto',   # OCR mode for pdf2md ('auto','on','off')
               engine=None,      # pdf_oxide OcrEngine for scanned pages
               **kwargs
               ) -> L:
	'Markdown-chunked text from each page (OCR fallback via `pdf2md`); yields (page, chunk_idx, text) tuples.'
	return L((pg, ci, chunk)
	         for pg, md in enumerate(self.pdf2md(st, end, ocr=ocr, engine=engine))
